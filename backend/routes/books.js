...
router.get('/', async (req, res) => {
  try {
    ...
    const books = await Book.find(filter).sort({ genre: 1, title: 1 });
    res.json(books);
  } catch (err) {
...
// GET /api/books/code/:bookId  (lookup by human-readable ID, used by QR scan / manual entry)
router.get('/code/:bookId', async (req, res) => {
  try {
    const book = await Book.findOne({ bookId: req.params.bookId.toUpperCase() });
    if (!book) return res.status(404).json({ message: 'No book found with that ID.' });
    res.json(book);
  } catch (err) {
...
router.get('/:id/qr', async (req, res) => {
  try {
    ...
    res.json({ dataUrl, payload: `BOOK:${book.bookId}` });
  } catch (err) {
...
router.post('/', async (req, res) => {
  try {
    const { title, author, genre, house, totalCopies, availableCopies, icon } = req.body;
    if (!title || !author || !genre || !house || totalCopies == null) {
      return res.status(400).json({ message: 'title, author, genre, house and totalCopies are required.' });
    }
    ...
    const book = await Book.create({
      ...
      house,
      totalCopies,
      availableCopies: availableCopies != null ? Math.min(availableCopies, totalCopies) : totalCopies,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // Guard: don't let totalCopies drop below copies currently on loan.
    ...
    if (house != null) book.house = house;
    if (icon != null) book.icon = icon;
    if (totalCopies != null) {
    ...
    if (availableCopies != null) {
      book.availableCopies = Math.max(0, Math.min(book.totalCopies, availableCopies));
    }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const activeLoans = await Transaction.countDocuments({ book: book._id, status: 'issued' });
    if (activeLoans > 0) {
      return res.status(400).json({
    ...
    res.json({ message: 'Book deleted', bookId: book.bookId });
  } catch (err) {
